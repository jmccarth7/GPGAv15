...
do  i_GP_Generation=1,n_GP_Generations
    ...
    if( i_GP_Generation .eq. 1) then
        ...
        if( myid == 0 )then
            ...
            !debug only     GP_Adult_Population_Node_Type(i_GP_individual,:,:) = GP_Node_Type_Answer(:,:) ! debug only
            !debug only enddo 
    ...
    do  i_GP_individual=1,n_GP_individuals
        ...
        if( Run_GP_Calculate_Fitness(i_GP_Individual) ) then
            ...
            if( myid == 0 )then
                write(*,'(/A,1x,I6/)') &
                 '0: aft call GPCODE_GA_lmdif_Parameter_Optimization routine', myid
            endif ! myid == 0
            ...
            if( myid == 0 )then
                write(6,'(/A,2(1x,I6),2(1x,E15.7))') &
                      '0: i_GP_gen, i_GP_Indiv, xcount, GP_Child_Indiv_SSE(i_GP_Indiv) ', &
                          i_GP_generation, i_GP_Individual, xcount, GP_Child_Individual_SSE(i_GP_Individual)
            ...
            if( myid == 0 )then
                ...
                write(6,'(/A)')&
                  '0: after GPCODE_GA_lmdif_Parameter_Optimization routine'

                write(6,'(/A,2(1x,I6), 1x, E15.7)')&
                      '0:2 i_GP_gen, i_GP_indiv, GP_pop_fit(i_GP_indiv) ', &
                           i_GP_generation, i_GP_individual, GP_population_fitness(i_GP_individual)
